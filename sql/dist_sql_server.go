...
func (ds *distSQLServerImpl) SetupFlows(ctx context.Context, req *SetupFlowRequest) (
	*EmptyResponse, error,
) {
	txn := ds.setupTxn(ctx, &req.Flow.Txn)
	reader, err := NewTableReader(req.Flow.Reader, txn, ds.evalCtx)
	if err != nil {
		return nil, err
	}
	pErr := reader.Run()
