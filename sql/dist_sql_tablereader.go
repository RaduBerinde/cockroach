...
func NewTableReader(spec *TableReaderSpec, txn *client.Txn, evalCtx parser.EvalContext) (
	*tableReader, error,
	...
	outputCols := make([]int, len(spec.OutputColumns))
	...
	for _, c := range outputCols {
		if c < 0 || c >= numCols {
			return nil, fmt.Errorf("invalid column index %d", c)
		}
	...
	for i, s := range spec.Spans {
		tr.spans[i] = span{start: s.Span.Key, end: s.Span.EndKey, count: s.Count}
	}
