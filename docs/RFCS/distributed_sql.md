...
with the table, but they can be used for any join in the right circumstances. We
introduce a variant of `TableReader` which has an input stream. Each element of
the input stream drives a point-lookup in another table or index, with a
corresponding value in the output. Internally the aggregator performs lookups in
batches, the way we already do it today. Example:
...
In terms of the physical implementation of `JOIN-READER`, there are two possibilities:
 ...
 2. it can use routers-by-range to route each input to an instance of
   `JOIN-READER` on the node for the respective range; the flow of data
    ...
    needs to go through a certain flow.

    Even with this optimization, it might be too expensive to set up the remote
    flows if we only end up doing a handful of queries. We can investigate a
    hybrid approach where we batch some results and depending on how many we
    have and how many different ranges/nodes they span, we choose
...
routers:
 ...
 - the routers can *duplicate* all rows from the physical streams for one table
   ...
   for subqueries, e.g. `SELECT .. WHERE x IN (SELECT ..)`
