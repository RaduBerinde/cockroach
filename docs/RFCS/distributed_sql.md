...
- `SORT` sorts the input according to a configurable set of columns. Note that
  ...
  ordering, instead it just adds an intra-stresam ordering to the logical
  output stream (and to each one of the potentially maby physical output
  streams). The global ordering, when needed, is done by an input synchronizer
  of a grouped processor (such as `LIMIT` or `FINAL`).
...
- `INTENT-COLLECTOR` is a single-group aggregator, scheduled on the gateway,
  that received all the intents generated by a `MUTATE` and keeps track of them
  in memory until the transaction is committed.
- `FINAL` is a single-group aggregator, scheduled on the gateway, that converts
  its input into a pgwire result set.
