...
func mtStartSQLFlagsInit(cmd *cobra.Command) error {
	...
	for i := range serverCfg.Stores.Specs {
		...
		if spec.BallastSize == nil {
			...
			// with.
			zero := storageconfig.SizeSpec{Bytes: 0, Percent: 0}
...
// sizeFlagVal is a pflag.Value wrapper for storageconfig.SizeSpec. It can be
// set to an absolute bytes amount or a percentage.
type sizeFlagVal struct {
	spec *storageconfig.SizeSpec
}

var _ pflag.Value = &sizeFlagVal{}

func newSizeFlagVal(spec *storageconfig.SizeSpec) *sizeFlagVal {
	return &sizeFlagVal{spec: spec}
}

// String returns a string representation of the SizeSpec. It is part of the
// pflag.Value interface.
func (sv *sizeFlagVal) String() string {
	if sv.spec.Percent != 0 {
		return string(crhumanize.Float(sv.spec.Percent, 6)) + "%"
	}
	return string(crhumanize.BytesExact(sv.spec.Bytes))
}
