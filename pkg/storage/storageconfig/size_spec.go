...
// SizeSpecConstraints describes acceptable values when parsing a SizeSpec
...
// ParseSizeSpec parses a string into a SizeSpec. The string contains one of:
...
//     equivalent to "10%".
func ParseSizeSpec(value string, constraints SizeSpecConstraints) (SizeSpec, error) {
	if fractionRegex.MatchString(value) {
		...
		if err != nil {
			return SizeSpec{}, errors.Wrapf(err, "could not parse size (%s)", redact.Safe(value))
		}
		...
		if percent < minPercent || percent > maxPercent {
			return SizeSpec{}, errors.Newf(
				...
				crhumanize.Float(minPercent, 6), crhumanize.Float(maxPercent, 6), redact.Safe(maxPercent),
			)
		}
		return SizeSpec{Percent: percent}, nil
	}
	...
	if bytes < constraints.MinBytes {
		return SizeSpec{}, errors.Newf("size (%s) must be at least %s",
