...
func TestParseInitNodeAttributes(t *testing.T) {
	...
	cfg.Stores = base.StoreSpecList{Specs: []base.StoreSpec{{InMemory: true, Size: storageconfig.SizeSpec{Bytes: base.MinimumStoreSize * 100}}}}
...
func TestCreateEnginesWithMultipleStores(t *testing.T) {
	...
	cfg.Stores = base.StoreSpecList{Specs: []base.StoreSpec{
		{Size: storageconfig.SizeSpec{Bytes: base.MinimumStoreSize}, Path: tmpDir1},
		{Size: storageconfig.SizeSpec{Bytes: base.MinimumStoreSize}, Path: tmpDir2},
		{InMemory: true, Size: storageconfig.SizeSpec{Bytes: base.MinimumStoreSize * 100}},
	}}
	engines, err := cfg.CreateEngines(context.Background())
	if err != nil {
		t.Fatalf("Failed to initialize stores: %s", err)
