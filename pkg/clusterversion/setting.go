...
// minSupportedVersion as the cluster could've started up earlier and
// enabled features that are no longer compatible it; we can't assume it's our
// latestVersion as that would enable features that may trip up older versions
...
func (cv *clusterVersionSetting) initialize(
	...
	if ver := cv.activeVersionOrEmpty(ctx, sv); ver != (ClusterVersion{}) {
		...
		// is first initialized to MinSupportedVersion and then
		// re-initialized to BootstrapVersion (=LatestVersion).
