...
// createPlanForRender takes a PhysicalPlan that and updates it to produce
// results corresponding to the render node. An evaluator stage is added if the
...
func (dsp *DistSQLPlanner) createPlanForRender(
	...
	err = p.AddRendering(n.render, planCtx, p.PlanToStreamColMap, typs,
		dsp.convertOrdering(n.reqOrdering, newColMap),
	)
	if err != nil {
		return err
	}
